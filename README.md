...
> 📄 **이 프로젝트를 어떻게 분석·개선했는지는 [PORTFOLIO.md](PORTFOLIO.md)를 참고하세요** — 위협 모델, Before/After 개선 사례, 트러블슈팅 과정, 아키텍처·시퀀스 다이어그램을 실제 커밋 근거와 함께 정리했습니다.

## 👤 담당 파트 (howl213)

이 프로젝트는 팀 프로젝트이며, 처음 파트를 나눌 때 **챗봇(본인, howl213) / 프론트엔드(Sunjung Hwang) / 백엔드·WAS(yysong1249)**로 역할을 정했습니다. `chatbot-service`(챗봇 로직·보안)는 본인이 처음 설계·구현했고, 이후 팀원들이 함께 개선/보완했습니다. `chatbot` 브랜치 기준 본인 커밋 21건을 주제별로 정리하면 다음과 같습니다.
