...
// ── 환자용 "내 진료기록" 조회 ──────────────────────────────────────
// 관리자용 목록(GET /)과 달리 requirePermission을 쓰지 않고, 로그인 여부 + 본인 소유 여부만 확인한다.
// board.js/chat.js와 동일한 패턴: URL 파라미터가 아니라 세션의 patientId를 WHERE 절에 직접 사용해,
// 애초에 "내 것"이 아닌 행은 쿼리 결과에 나올 수조차 없게 만든다 (IDOR 방지).

// 목록: 요약 정보만 (원문 OCR 텍스트는 목록엔 안 실음 - 상세 조회에서만 반환)
router.get("/mine", async (req, res) => {
  ...
  try {
    ...
    console.error("[documents mine list error]", err);
    res.status(500).json({ message: "서버 오류가 발생했습니다." });
  }
});

// 상세: 원문(extracted_text)까지 포함. WHERE 절에 patient_id를 함께 걸어,
// 다른 환자의 문서 id를 넣어도 0건으로 응답되게 한다 (URL의 id만으로는 절대 타인 문서에 접근 불가).
router.get("/mine/:id", async (req, res) => {
  ...
  try {
    ...
    console.error("[documents mine detail error]", err);
    res.status(500).json({ message: "서버 오류가 발생했습니다." });
  }
});
